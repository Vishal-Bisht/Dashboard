...
const CustomerChat = ({ selectedId, aiChatCollapsed, onExpandAiChat, isMobile, onOpenAiChat, onBack, showBackButton }) => {
	...
	return (
		<section className="flex-1 flex flex-col bg-white h-full min-h-screen relative">
			{/* Chat Header */}
			<div className="flex justify-between items-center px-2 md:px-4 h-14 md:h-16 border-b border-[#e1e2e6]">
				<div className="flex items-center gap-2">
					{/* Back button for mobile */}
					{showBackButton && (
						<button className="p-1 hover:bg-gray-100 rounded block md:hidden" title="Back to Inbox" onClick={onBack}>
				...
				<div className="flex gap-2 md:gap-3 items-center">
					...
					{isMobile && (
						<button className="p-1 hover:bg-gray-100 rounded block md:hidden" title="Open Copilot Panel" onClick={onOpenAiChat}>
							<Bot className="w-5 h-5 text-[#6364c8]" />
						</button>
					)}
					{/* PanelRight icon */}
					{!isMobile && aiChatCollapsed && (
						<button className="p-1 hover:bg-gray-100 rounded hidden md:block" title="Open Copilot Panel" onClick={onOpenAiChat}>
					...
					<button className="p-1 hover:bg-gray-100 rounded" title="Dark Mode">
				...
				{filteredMessages.map((msg, idx) => {
					if (msg.type === 'customer') {
						return (
							<div
								key={msg.id}
								className="bg-[#f1f0f0] rounded-2xl mb-2 md:mb-4 max-w-[80%] self-start p-2 md:p-4 text-sm md:text-md"
							>
								{msg.text}
								<div className="text-[10px] md:text-xs text-gray-500 mt-1 md:mt-2 ml-1 md:ml-2">
									{msg.time}
								</div>
							</div>
						);
						...
						return (
							<div className="flex items-end justify-end" key={msg.id}>
								<div
									className="bg-[#dfe7ff] rounded-2xl mb-2 md:mb-4 max-w-[80%] self-end p-2 md:p-4 text-xs md:text-sm"
								>
									{msg.text}
									<div className="flex items-center justify-end mt-1 md:mt-2 gap-1 md:gap-2 text-[10px] md:text-xs text-gray-500">
										{msg.seen && (
