...
const Inbox = ({ selectedId, onSelect }) => {
	...
	return (
		<aside className="bg-white border-r border-[#e1e2e6] flex flex-col h-full z-20 w-full md:w-[320px] min-w-0 md:min-w-[320px] max-w-full md:max-w-[320px] scrollbar-hide">
			<div className="h-16 px-4 flex items-center font-bold border-b border-[#e1e2e6] justify-between text-base md:text-lg lg:text-xl relative">
				<span>Your inbox</span>
				{/* For Mobile screen size filters: right side of topbar */}
				<div className="flex md:hidden gap-1 ml-2 absolute right-4 top-1/2 -translate-y-1/2">
					{/* Open filter dropdown */}
					<div className="relative">
						<button
							className="bg-white rounded px-2 py-1 text-xs md:text-sm flex items-center gap-1 cursor-pointer font-semibold min-w-0 max-w-[120px] truncate"
					...
					<div className="relative">
						<button
							className="bg-white rounded px-2 py-1 text-xs md:text-sm flex items-center gap-1 cursor-pointer font-semibold min-w-0 max-w-[150px]"
			...
			<div className="md:flex hidden gap-1 px-2 py-2">
				{/* Open filter dropdown */}
				<div className="relative">
					<button
						className="bg-white rounded px-2 py-1 text-xs md:text-sm flex items-center gap-1 cursor-pointer font-semibold min-w-0 max-w-[120px] truncate"
			...
			<div className="conversation-list flex-1 px-1 overflow-y-auto scrollbar-hide">
				{conversations.map((conv) => (
					<div
						...
						className={`px-4 py-3 border-b border-[#e1e2e6] cursor-pointer ${
							selectedId === conv.id
					...
					>
						<div className="flex items-center mb-1">
							<div
								className={`w-7 h-7 md:w-9 md:h-9 rounded-full flex items-center justify-center text-md md:text-md font-bold mr-2 shrink-0 ${conv.avatarColor}`}
							>
								{conv.avatar}
							</div>
